exports = async function(phoneNumber, message){
    ...
    try {
      ...
      // add if we need to test whether the test actully exists. Like if use types wrong number?
      ...
      var tester = await recipientsCollection.updateOne({'number':phoneNumber}, {$addToSet:{'testReponses':testid}});
    }
    catch(err){
          console.error(err);
